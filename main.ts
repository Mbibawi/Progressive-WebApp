 ...
 */
function modifyUserLanguages(lang: string):boolean {
	if (userLanguages.indexOf(lang) > -1) {
...
async function loadFonts() {
	...
	await AvaShenouda.load();
	await ArialCoptic.load();
...
function getPrayerFromInputBox() {
	...
	function createHtmlElementForPrayer(
		...
		let dataRoot: string = firstElement.replace(/Part\d+/, '');
 ...
 */
async function 	showTitlesInRightSideBar(titlesCollection: NodeListOf<Element>, rightTitlesDiv: HTMLElement, btn:Button, clear:boolean = true){
	//this function shows the titles in the right side Bar
	if (clear) { rightTitlesDiv.innerHTML = '' }; //we empty the side bar
	let bookmark: HTMLAnchorElement, div: HTMLDivElement;
	for (let i = 0; i < titlesCollection.length; i++){
		...
		addTitle(titlesCollection[i] as HTMLElement);
	};

	function addTitle(titles: HTMLElement) {
		let text: string = '';
 ...
 * @returns 
 */
function showChildButtonsOrPrayers(btn: Button, clear: boolean = true, click: boolean = true, pursue: boolean = true) {
	if (!btn) { return };
	let btnsDiv: HTMLElement = leftSideBar.querySelector('#sideBarBtns');
	if (clear) { btnsDiv.innerHTML = ''; inlineBtnsDiv.innerHTML = '' };
...
/**
	...
	async function createGoBackBtn(
			goTo: Button, div: HTMLElement, cssClass:string, bookmarkID?:string) {
...
/**
	 ...
	 */
	function createBtn(btn: Button, btnsBar: HTMLElement, btnClass?: string, clear:boolean = true): HTMLElement {
...
function toggleClassListForAllChildrenOFAnElement(ev: Event, myClass: string) {
	...
	let hasDataLang= containerDiv.querySelectorAll('[data-lang]') as NodeListOf<HTMLElement>;
 ...
 * @param btn 
 */
async function showPrayers(btn: Button, clearSideBar: boolean = true) {
	...
	if (fractions.length > 0) {
		let insertion = containerDiv.querySelector('[data-root=\"PMCFractionPrayerPlaceholder&D=0000\"]') as HTMLElement; //this is the id of the html element after which we will insert the inline buttons for the fraction prayers
 ...
 */
	function clearDivs() {
		...
		if (clearSideBar) { rightSideBar.querySelector('#sideBarBtns').innerHTML = '' }; //this is the right side bar where the titles are displayed for navigation purposes
	};
};
 ...
 */
async function setCSSGridTemplate(container: HTMLElement) {
	...
	*/
	...
	function setGridAreas(row: HTMLElement): string {
			let areas: string[] = [], child: HTMLElement;
...
function collapseText(element: HTMLElement){
	let siblings: HTMLElement[] = [], next: Element;
 ...
 */
async function showInlineButtonsForFractionPrayers(btn: Button, fractions: string[][][], btnsDiv: HTMLElement) {
	let selected: string[][][] = [], title: {tableID:string, class:string}, fractionBtn: Button, inlineBtn: Button;
	...
	fractionBtn = new Button({
		...
		pursue:false, //We set it to false in order to prevent showchildButtonsOrPrayers() from creating htm buttons for all the inline buttons of fraction button. We will display them by group of 6 when the onClick() is triggered
		inlineBtns:[],
		cssClass: inlineBtnClass,
		onClick: () => {
			...
			if ((fractionBtn.inlineBtns.length / 6) <= 1) {
				...
				splitFractions(0);
				function splitFractions(i:number) { 
					for (let n = i; n < (i + 6); n++){
	...
	* @param {Button} target - the button to which the inline buttons that will be created will be attached as members of its inlineBtns[] property
	 */
	async function createInlineBtns(target:Button) {
			...
			inlineBtn = new Button(
				{
					...
					label: {
						AR: fractionTable[0][btn.languages.indexOf('AR') + 1],//fractionTable[0] is the first row of the Word table from which the text of the fraction prayer was retrieved. The 1st row contains  the title of the fraction prayer in each of the available langues. We look for the Arabic title by the index of 'AR' in the btn.languages property. We add 1 to the index because the string[] containing the titles starts with the id of the table
	 ...
	 */
	function findAndProcessPrayers(p: string, btn:Button, div:HTMLDivElement = containerDiv) {
		let tblTitle: string;
		btn.prayersArray.map(wordTable => {
		if (!wordTable[0]) {
			return
		...
		if (p == tblTitle) {
			...
			if (RegExp(tblTitle.split('&D=')[0]).test(p.split('&D=')[0])) {				
				let dates: string[] = tblTitle.split('&D=(')[1].split(')')[0].split('||');
...
function showSettingsPanel() {
	...
	(function showDatePicker() {
		let datePicker: HTMLInputElement = createBtn('input', undefined, undefined, undefined, inlineBtnsDiv, 'datePicker', undefined, 'date', undefined, undefined,
			{
				event: 'change',
	...
	function addLanguage(id: string, dataSet: string, innerText: string) {
		btn = createBtn(
			'button',
			undefined,
			'addLang',
			innerText,
			inlineBtnsDiv.querySelector('#langsContainer'),
			id, dataSet,
			'submit',
			undefined,
			'red',
	...
	(async function showExcludeActorButon (){
		...
		inlineBtnsDiv.appendChild(container);
		actors.map(actor => {
			if (actor == 'CommentText') { return }; //we will not show a button for 'CommentText' class, it will be handled by the 'Comment' button
			let show: boolean = showActors.get(actor);
			btn = createBtn('button', 'button','settingsBtn', 'remove ' + actor, container, actor, actor, undefined, undefined, undefined,
				{
					...
					fun: () => {
						...
						if (show == false) { btn.classList.add('langBtnAdd') };
						if (actor == 'Comment') { showActors.set('CommentText', show) }
						showChildButtonsOrPrayers(lastClickedButton);
	...
	function createBtn(tag:string, role: string=tag, btnClass:string, innerText:string, parent:HTMLElement, id?:string, dataSet?:string, type?:string, size?:string, backgroundColor?:string, onClick?:{event:string, fun:Function}):HTMLElement {
...
async function insertRedirectionButtons(querySelector:string, btns: Button[], position:InsertPosition = 'beforebegin'){
...
async function fetchSynaxarium() {
    let date: string = '1_9';
    let url: string = 'https://www.copticchurch.net/synaxarium/' + `${date}` + '.html?lang=ar';
